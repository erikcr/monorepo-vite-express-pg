...
Statuses: `[ ]` not started · `[~]` in progress · `[ ]` done (committed + pushed)
...
### 0.4 `[ ]` Liveness vs readiness split
...
### 1.1 `[ ]` Auth scaffold (provider-agnostic)
...
### 1.3 `[ ]` Complete CRUD example + pagination convention
...
### 2.1 `[ ]` Validation helper middleware
...
### 2.2 `[ ]` Request logging + request IDs + error-tracking seam
...
### 2.4 `[ ]` React ErrorBoundary + QueryClient defaults
...
### 3.1 `[ ]` Serve API reference at /docs
